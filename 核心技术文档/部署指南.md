...
应用图标需要放在以下目录中，每个密度都需要提供对应尺寸的图标：

**项目路径**: `app/src/main/res/`

#### 7.1.1 应用启动图标 (mipmap 目录)

应用启动图标需要放在以下目录，每个目录对应不同的屏幕密度：

1. **mipmap-mdpi/** (48x48 dp)
   - 文件: `ic_launcher.png` 和 `ic_launcher_round.png`
   - 实际尺寸: 48x48 像素
   - 完整路径: `app/src/main/res/mipmap-mdpi/ic_launcher.png`

2. **mipmap-hdpi/** (72x72 dp)
   - 文件: `ic_launcher.png` 和 `ic_launcher_round.png`
   - 实际尺寸: 72x72 像素
   - 完整路径: `app/src/main/res/mipmap-hdpi/ic_launcher.png`

3. **mipmap-xhdpi/** (96x96 dp)
   - 文件: `ic_launcher.png` 和 `ic_launcher_round.png`
   - 实际尺寸: 96x96 像素
   - 完整路径: `app/src/main/res/mipmap-xhdpi/ic_launcher.png`

4. **mipmap-xxhdpi/** (144x144 dp)
   - 文件: `ic_launcher.png` 和 `ic_launcher_round.png`
   - 实际尺寸: 144x144 像素
   - 完整路径: `app/src/main/res/mipmap-xxhdpi/ic_launcher.png`

5. **mipmap-xxxhdpi/** (192x192 dp)
   - 文件: `ic_launcher.png` 和 `ic_launcher_round.png`
   - 实际尺寸: 192x192 像素
   - 完整路径: `app/src/main/res/mipmap-xxxhdpi/ic_launcher.png`

6. **mipmap-anydpi-v26/** (自适应图标，Android 8.0+)
   - 文件: `ic_launcher.xml` 和 `ic_launcher_round.xml`
   - 这些是 XML 文件，定义了自适应图标的背景和前景
   - 完整路径: `app/src/main/res/mipmap-anydpi-v26/ic_launcher.xml`

#### 7.1.2 自适应图标资源 (drawable 目录)

自适应图标需要以下资源文件：

1. **drawable/ic_launcher_background.xml**
   - 完整路径: `app/src/main/res/drawable/ic_launcher_background.xml`
   - 这是图标的背景层（通常是纯色或渐变）

2. **drawable-v24/ic_launcher_foreground.xml**
   - 完整路径: `app/src/main/res/drawable-v24/ic_launcher_foreground.xml`
   - 这是图标的前景层（通常是应用 Logo）

### 7.2 如何替换应用图标

#### 方法一：使用 Android Studio 的 Image Asset Studio（推荐）

1. 在 Android Studio 中，右键点击 `res` 文件夹
2. 选择 `New` → `Image Asset`
3. 在 `Icon Type` 中选择 `Launcher Icons (Adaptive and Legacy)`
4. 选择你的图标图片（建议使用 1024x1024 像素的 PNG 图片）
5. 调整前景和背景
6. 点击 `Next` → `Finish`
7. Android Studio 会自动生成所有需要的图标文件

#### 方法二：手动替换

1. **准备图标文件**
   - 准备一个 1024x1024 像素的 PNG 图标（透明背景）
   - 使用在线工具（如 [Android Asset Studio](https://romannurik.github.io/AndroidAssetStudio/icons-launcher.html)）生成不同密度的图标

2. **替换文件**
   - 将生成的图标文件复制到对应的 `mipmap-*` 目录
   - 确保文件名正确：`ic_launcher.png` 和 `ic_launcher_round.png`

3. **更新自适应图标**（可选，Android 8.0+）
   - 编辑 `drawable/ic_launcher_background.xml` 设置背景色
   - 编辑 `drawable-v24/ic_launcher_foreground.xml` 设置前景图标
...
- **支持格式**: PNG, JPG, XML (Vector Drawable), WebP
- **命名规则**: 小写字母、数字、下划线，不能以数字开头
- **示例**: 
  - `app/src/main/res/drawable/ic_menu.png`
  - `app/src/main/res/drawable/button_background.xml`

#### 7.3.2 不同密度的 Drawable

如果需要为不同屏幕密度提供不同尺寸的图片：
...
| 用途 | 推荐尺寸 | 格式 | 位置 |
|------|---------|------|------|
| 应用图标 | 1024x1024 | PNG | mipmap-* |
| 界面图标 | 24x24, 48x48 dp | PNG/SVG | drawable |
| 启动页图片 | 1080x1920 | PNG/JPG | drawable |
| 背景图片 | 根据设计 | PNG/JPG/WebP | drawable |

### 7.4 图片资源优化建议

1. **使用 WebP 格式**: 比 PNG 体积更小，质量更好
2. **使用 Vector Drawable**: 对于简单图标，使用 XML 格式的矢量图
3. **压缩图片**: 使用工具（如 TinyPNG）压缩图片
4. **按需提供**: 只为需要的密度提供图片，避免 APK 体积过大

## 8. XML 文件问题排查

### 8.1 "Valid XML document must have a root tag" 错误
